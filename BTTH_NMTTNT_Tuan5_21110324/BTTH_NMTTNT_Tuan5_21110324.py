...
class Graph:
    ...
    def primMST(self, d_temp, t):
        ...
        for c in range(self.V):
            ...
            # current distanceis greater than new distance and
            # the vertex in not in the shortest path tree
            for v in range(self.V):
                # graph[u][v] is non zero inly for adjacent vertices of m
...
# Detemine weight of that mst and connect it with the visited node and 0th node
...
def heuristic(tree, p_id, t, V, graph):
    ...
    if num != 0:
        ...
        # new sequential no as value MST to work
        for i in range(V):
            if i not in visited:
                d_temp[i] = key
                key = key + 1
...
def startTSP(graph, tree, V):
    ...
    while goalState == 0:
        ...
        # If node to expand is ) and path is complete, we are done
        # We check node at the time of expansion and not at the time od generation
        if toExpand.c_no == 0 and path == 1:
            goalState = 1
            cost = toExpand.f_value  # Total actual cost incured
        else:
